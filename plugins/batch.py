    ...
    try:
        if lt == 'public':
            try:
                ...
                # Add @ prefix if it looks like a username
                if not chat_id.startswith("@") and not chat_id.startswith("-") and not chat_id.isdigit():
                    chat_id = f"@{chat_id}"

                # Case 1: If it's a bot username
                if chat_id.lower().endswith("bot"):
                    emp[chat_id] = False
                    xm = await u.get_messages(chat_id, d)
                    emp[chat_id] = getattr(xm, "empty", False)
                    if not emp[chat_id]:
                        emp[chat_id] = True
                        print(f"✅ Bot chat {chat_id} found successfully")
                        return xm

                # Case 2: Try with client `c`
                xm = await c.get_messages(chat_id, d)
                print(f"✅ fetched by {c.me.username}")
                emp[chat_id] = getattr(xm, "empty", False)

                if emp[chat_id]:
                    print(f"⚠️ Not fetched by {c.me.username}, retrying with {u.me.username}")
                    try:
                        await u.join_chat(chat_id)
                    except Exception as e:
                        print(f"Join failed: {e}")

                    # Try again with `u`
                    chat_obj = await u.get_chat(chat_id)
                ...
                import traceback
                print(f'❌ Error fetching public message: {type(e).__name__} - {e}')
                traceback.print_exc()
                return None
